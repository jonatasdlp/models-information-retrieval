	...
	class Vocabulary
	  ...
	  def self.probabilistic(count, df)
	    Math.log2(0.5/(1 - 0.5)) + Math.log2((1-(df/count.to_f))/((df/count.to_f))).round(3)
	  end

	  def self.probabilistic_new(query, documents)
	  	q = Vocabulary.query_in_documents(query, documents)
	  	if q > 0
	    	Math.log2((q/4.to_f)/(1 - (q/4.to_f))) + Math.log2((1-(q/4.to_f))/(q/4.to_f)).round(3)
	    else
	    	0
	    end
	  end
